 ...
 * 向服务器提交语言切换请求
 */
async function submitLangChange(lang: App.I18n.LangType): Promise<boolean> {
  if (!props.submitUrl) return true;

  // 使用系统标准请求
  const { error } = await post(props.submitUrl, { locale: lang }, {
...
async function changeLang(lang: App.I18n.LangType) {
  if (props.translationsUrl) {
    const { data, error } = await get<Record<string, any>>(props.translationsUrl, { locale: lang }, { showErrorMessage: false });
    if (!error && data) registerLocaleMessages(lang, data);
  }

  // 提交到服务器
  const submitted = await submitLangChange(lang);
  ...
  if (submitted && props.reloadOnChange) {

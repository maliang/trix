...
import { addThemeVarsToGlobal, createThemeToken, getBaseUrl, getNaiveTheme, themeSettings as defaultThemeSettings, toggleAuxiliaryColorModes, toggleCssDarkMode } from './shared';
/** 主题 Store */
export const useThemeStore = defineStore(SetupStoreId.Theme, () => {
    const scope = effectScope();
    const osTheme = usePreferredColorScheme();
    /** 主题设置 */
    const settings = ref({ ...defaultThemeSettings });
    /** 水印时间实例 */
     ...
     */
    async function loadRemoteThemeConfig(force = false) {
        // 如果未配置 API 地址，直接返回
        if (!THEME_CONFIG_API) {
        ...
        const { data, error } = await get(THEME_CONFIG_API, {}, {
            withToken: false,
     ...
     * 保存主题配置到服务器
     */
    async function saveRemoteThemeConfig() {
        if (!THEME_CONFIG_API) {
            console.log('[ThemeStore] 未配置主题配置 API 地址，无法保存');
            return false;
        }
        // 使用系统标准请求
        const { error } = await post(THEME_CONFIG_API, settings.value, {
            showErrorMessage: false
        });
        if (error) {
     ...
     */
    function setWatermarkEnableTime(enable) {
    ...
    scope.run(() => {
        ...
        // 监听水印设置以控制计时器
        watch(() => [settings.value.watermark.visible, settings.value.watermark.enableTime], () => {
        ...
        // 监听混合子菜单背景色变化
        watch(() => settings.value.sider.mixChildMenuBgColor, () => {
            updateMixChildMenuBgColor();
        }, { immediate: true });
    });
    /** 作用域销毁时 */

...
import { filterRoutesByRoles, sortRoutesByOrder, getMenusByRoutes, getCacheRouteNames, getExcludeCacheRouteNames, getSelectedMenuKeyPath, getBreadcrumbsByRoute, transformMenuToSearchMenus, transformMenuRoutesToRoutes, extractBuiltinRouteMetas } from './shared';
/**
 * 路由 Store
 * 管理路由状态、菜单、面包屑等
 */
export const useRouteStore = defineStore(SetupStoreId.Route, () => {
    ...
    const baseExcludeCacheRoutes = ref([]);
    /**
     * 获取缓存路由
     * @param routes 路由列表
     */
     ...
     */
    function removeCacheRoute(routeName) {
        // 从 cacheRoutes 中移除
     ...
     * @param routeName 路由名称
     */
    function restoreCacheRoute(routeName) {
     ...
     */
    function updateBuiltinRoutesMeta(menuRoutes) {
        const builtinMetas = extractBuiltinRouteMetas(menuRoutes);
        if (builtinMetas.size === 0)
            return;
        // 遍历内置路由，更新其 meta 信息
        builtinRoutes.forEach(route => {
            ...
            if (!childRoute?.name)
                return;
            const routeName = childRoute.name;
            ...
            if (apiMeta && childRoute.meta) {
                ...
                if (apiMeta.title)
                    childRoute.meta.title = apiMeta.title;
                if (apiMeta.icon)
                    childRoute.meta.icon = apiMeta.icon;
                if (apiMeta.schemaSource)
                    childRoute.meta.schemaSource = apiMeta.schemaSource;
                if (apiMeta.useJsonRenderer !== undefined)
                    childRoute.meta.useJsonRenderer = apiMeta.useJsonRenderer;
                if (apiMeta.layoutType)
                    childRoute.meta.layoutType = apiMeta.layoutType;
            }
        });
    }
    /**
    ...
    async function initAuthRoute() {
        ...
        if (menuRouteUrl) {
            // 从 API 获取菜单路由
            try {
                const menuRoutes = await fetchMenuSchema(menuRouteUrl);
    ...
    /** 登录后默认显示的路由列表 */
    const defaultAfterLoginRoutes = ref([]);
    /**
     * 收集所有 isDefaultAfterLogin: true 的路由
     * @param routes 路由列表
     * @returns 默认显示的路由列表
     */
    function collectDefaultAfterLoginRoutes(routes) {
        const result = [];
        function traverse(routeList, parentPath = '') {
     ...
     */
    function initDefaultTabs(routes, defaultRoutes) {
        ...
        if (defaultRoutes.length === 0) {
            ...
            if (isBlankLayout) {
                ...
                tabStore.addDefaultTab({
                    name: lastDefault.route.name,
                    path: lastDefault.route.path,
                    fullPath: lastDefault.fullPath,
                    meta: {
                        title: lastDefault.route.meta?.title || '',
                        icon: lastDefault.route.meta?.icon
                    }
                });
            }
            // 初始化固定标签页（添加到标签栏）
     ...
     */
    function collectFixedTabs(routes, parentPath = '') {
        const tabStore = getTabStore();
     ...
     * @param routes 路由列表
     */
    function initHomeTabFromRoutes(routes) {
     ...
     * @param parentPath 父路径
     */
    function getFullPath(routes, name, parentPath = '') {
        for (const route of routes) {
            const currentPath = route.path.startsWith('/')
                ? route.path
                : `${parentPath}/${route.path}`.replace(/\/+/g, '/');
     ...
     * @returns 菜单路径
     */
    function getSelectedMenuKeyPathByKey(selectedKey) {

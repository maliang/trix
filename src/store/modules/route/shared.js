 ...
 */
function createDynamicPage(name, schemaSource) {
 ...
 * @returns 过滤后的路由列表
 */
function filterRouteByRoles(route, roles) {
 ...
 * @returns 菜单列表
 */
export function getMenusByRoutes(routes, parentPath = '') {
    const menus = [];
 ...
 * @returns 菜单项
 */
function getMenuByRoute(route, fullPath) {
    ...
    const menu = {
        ...
        icon: SvgIconVNode({ icon: icon, fontSize: 20 })
    };
    return menu;
}
/**
...
export function getCacheRouteNames(routes) {
    const cacheNames = [];
    function collectCacheNames(routeList) {
 ...
 * @returns 排除缓存的路由名称列表
 */
export function getExcludeCacheRouteNames(routes) {
    const excludeNames = [];
    function collectExcludeNames(routeList) {
        routeList.forEach(route => {
            // 如果路由显式设置 keepAlive: false，则排除缓存
            if (route.component && route.name && route.meta?.keepAlive === false) {
                excludeNames.push(route.name);
            }
            // 递归处理子路由
            if (route.children?.length) {
 ...
 * @returns 菜单路径
 */
export function getSelectedMenuKeyPath(selectedKey, menus) {
    const keyPath = [];
    menus.some(menu => {
 ...
 */
export function extractBuiltinRouteMetas(menuRoutes) {
    const builtinMetas = new Map();
 ...
 */
export function transformMenuRoutesToRoutes(menuRoutes) {
    // 过滤掉内置路由，避免重复注册
    const filteredMenuRoutes = menuRoutes.filter(menuRoute => !BUILTIN_ROUTE_NAMES.includes(menuRoute.name));
    // 将菜单包装到 root 路由下
    const rootRoute = {
        path: '/',
        name: 'root',
 ...
 */
function transformMenuRouteToRoute(menuRoute, isChild = false) {

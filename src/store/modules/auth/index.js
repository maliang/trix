...
export const useAuthStore = defineStore(SetupStoreId.Auth, () => {
    ...
    /** 后台配置 */
    const backendConfig = ref(localStg.get('backendConfig') || {});
    /** 是否已登录 */
    ...
    async function getUserInfo() {
        ...
        const { data, error } = await get(userInfoApi, {}, { showErrorMessage: false });
        if (error || !data) {
    ...
    async function login(userName, password, redirect = true) {
        ...
        try {
            ...
            if (import.meta.env.VITE_AUTH_MOCK === 'Y') {
                ...
                const { data, error } = await post(loginApi, { username: userName, password });
                if (error || !data) {
                    return;
                }
                loginToken = {
            ...
            if (redirect) {
                ...
                const currentRoute = router.currentRoute.value;
                const queryRedirect = currentRoute.query.redirect;

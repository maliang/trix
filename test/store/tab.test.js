...
describe('Tab Store - 默认显示页面控制', () => {
    ...
    describe('Property 7: 默认显示页面控制', () => {
        ...
        it('固定标签页不能被关闭', () => {
            ...
            tabStore.addFixedTab(fixedRoute);
            // 检查是否可以关闭
        ...
        it('首页标签页永远不能被关闭', () => {
            ...
            tabStore.initHomeTab(homeRoute);
            // 检查首页是否可以关闭
     ...
     * Validates: Requirements 8.2
     */
    test.prop([fc.array(routeArbitrary, { minLength: 1, maxLength: 10 })])('对于任意数量的默认页面，只保留最后一个', (routes) => {
        setActivePinia(createPinia());
        const tabStore = useTabStore();
        // 添加所有路由作为默认页面
        routes.forEach(route => {
    ...
    test.prop([fc.array(routeArbitrary, { minLength: 1, maxLength: 5 })])('所有固定标签页都不能被关闭', (routes) => {
        setActivePinia(createPinia());
        const tabStore = useTabStore();
        // 添加所有路由作为固定标签页
        const uniqueRoutes = routes.filter((route, index, self) => index === self.findIndex(r => r.fullPath === route.fullPath));

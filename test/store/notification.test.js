...
describe('Notification Store - 属性测试', () => {
    ...
    describe('Property 1: 未读数量计算正确性', () => {
        test.prop([messageListArbitrary])('未读数量应等于 isRead 为 false 的消息数量', (messages) => {
            setActivePinia(createPinia());
        ...
        test.prop([messageListArbitrary, notificationMessageArbitrary])('添加新消息后未读数量应正确更新', (initialMessages, newMessage) => {
            setActivePinia(createPinia());
            const store = useNotificationStore();
        ...
        test.prop([messageListArbitrary])('按类型统计的未读数量之和应等于总未读数量', (messages) => {
            setActivePinia(createPinia());
            const store = useNotificationStore();
            store.setMessages(messages);
    ...
    describe('Property 2: 标签页筛选正确性', () => {
        test.prop([messageListArbitrary, messageTypeArbitrary])('切换标签页后，筛选的消息应只包含该类型', (messages, tabType) => {
            setActivePinia(createPinia());
            const store = useNotificationStore();
            store.setMessages(messages);
        ...
        test.prop([messageListArbitrary])('all 标签页应显示所有消息', (messages) => {
            setActivePinia(createPinia());
            const store = useNotificationStore();
            store.setMessages(messages);
        ...
        test.prop([messageListArbitrary, fc.array(messageTypeArbitrary, { minLength: 1, maxLength: 3 })])('配置了多个类型的标签页应显示所有匹配类型的消息', (messages, types) => {
            setActivePinia(createPinia());
            const store = useNotificationStore();
            store.setMessages(messages);
            // 设置标签页类型映射
        ...
        test.prop([messageListArbitrary, messageTypeArbitrary])('每个标签页的未读数量应等于该类型消息中未读的数量', (messages, tabType) => {
            setActivePinia(createPinia());
            const store = useNotificationStore();
            store.setMessages(messages);
    ...
    describe('Property 3: 标记已读正确性', () => {
        test.prop([
            ...
            fc.nat()
        ])('标记单条消息已读后，该消息的 isRead 应为 true', (messages, indexSeed) => {
            setActivePinia(createPinia());
            const store = useNotificationStore();
            store.setMessages(messages);
            // 选择一条消息进行标记
        ...
        test.prop([messageListArbitrary])('全部已读后，所有消息的 isRead 应为 true', (messages) => {
            setActivePinia(createPinia());
            const store = useNotificationStore();
            store.setMessages(messages);
            store.markAllAsRead();
        ...
        test.prop([messageListArbitrary, messageTypeArbitrary])('按类型全部已读后，该类型的所有消息 isRead 应为 true', (messages, targetType) => {
            setActivePinia(createPinia());
            const store = useNotificationStore();
            store.setMessages(messages);
        ...
        test.prop([messageListArbitrary])('标记已读后未读数量应正确减少', (messages) => {
            setActivePinia(createPinia());
            const store = useNotificationStore();
            store.setMessages(messages);
            const initialUnread = store.unreadCount;

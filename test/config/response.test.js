...
import { responseConfig, getValueByPath, parseResponse, parsePaginationData, createResponseConfig } from '@/config/response';
describe('响应格式解析 - Property 6', () => {
    ...
    describe('Property 6: 响应格式解析', () => {
        ...
        test.prop([responseArbitrary], { numRuns: 100 })('对于任意响应数据，parseResponse 应正确提取 code 字段', (response) => {
            const result = parseResponse(response);
            expect(result.code).toBe(response.code);
        });
        // 属性测试：对于任意响应数据，parseResponse 应正确提取 data 字段
        test.prop([responseArbitrary], { numRuns: 100 })('对于任意响应数据，parseResponse 应正确提取 data 字段', (response) => {
            const result = parseResponse(response);
            expect(result.data).toEqual(response.data);
        });
        // 属性测试：对于任意响应数据，parseResponse 应正确提取 message 字段
        test.prop([responseArbitrary], { numRuns: 100 })('对于任意响应数据，parseResponse 应正确提取 message 字段', (response) => {
            const result = parseResponse(response);
            expect(result.message).toBe(response.message);
        });
        // 属性测试：当 code 等于 successCode 时，success 应为 true
        test.prop([fc.anything(), fc.string()], { numRuns: 100 })('当 code 等于 successCode 时，success 应为 true', (data, message) => {
            const response = {
        ...
        test.prop([fc.integer().filter(n => n !== responseConfig.successCode), fc.anything(), fc.string()], { numRuns: 100 })('当 code 不等于 successCode 时，success 应为 false', (code, data, message) => {
            const response = { code, data, message };
            const result = parseResponse(response);
            expect(result.success).toBe(false);
        });
        // 属性测试：对于嵌套路径配置，应正确提取嵌套字段
        test.prop([nestedResponseArbitrary], { numRuns: 100 })('对于嵌套路径配置，应正确提取嵌套字段', (response) => {
            const nestedConfig = {
            ...
            const result = parseResponse(response, nestedConfig);
    ...
    describe('getValueByPath 函数测试', () => {
        ...
        test.prop([fc.string(), fc.anything()], { numRuns: 100 })('对于任意简单路径，应正确获取值', (key, value) => {
        ...
        // 属性测试：对于嵌套路径，应正确获取值
        test.prop([fc.string().filter(s => !s.includes('.') && s.length > 0),
            fc.string().filter(s => !s.includes('.') && s.length > 0),
            fc.anything()], { numRuns: 100 })('对于嵌套路径，应正确获取值', (key1, key2, value) => {
        ...
        test.prop([fc.string().filter(s => s.length > 0 &&
                !s.includes('.') &&
                !jsBuiltinProperties.includes(s))], { numRuns: 100 })('对于不存在的路径，应返回 undefined', (path) => {
    ...
    describe('parsePaginationData 函数测试', () => {
        ...
        test.prop([paginationDataArbitrary], { numRuns: 100 })('对于任意分页数据，应正确解析所有字段', (data) => {
            const result = parsePaginationData(data);
        ...
        test.prop([fc.array(fc.anything()), fc.nat(), fc.integer({ min: 1, max: 1000 }), fc.integer({ min: 1, max: 100 })], { numRuns: 100 })('对于自定义分页配置，应正确解析', (items, count, currentPage, size) => {
            const customConfig = {
                listField: 'items',
                totalField: 'count',
            ...
            const result = parsePaginationData(data, customConfig);
    ...
    describe('createResponseConfig 函数测试', () => {
        ...
        // 属性测试：自定义配置应覆盖默认值
        test.prop([fc.string(), fc.oneof(fc.integer(), fc.string()), fc.string(), fc.string()], { numRuns: 100 })('自定义配置应覆盖默认值', (codeField, successCode, dataField, messageField) => {
            const customConfig = {
                codeField,
                successCode,
                dataField,
        ...
        it('分页配置应正确合并', () => {
            ...
            const config = createResponseConfig({ pagination: customPagination });

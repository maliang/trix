...
import { createExpressionEvaluator, isExpressionSafe, } from 'vschema-ui';
// 创建测试用的求值器实例
...
describe('Property 8: 表达式安全性', () => {
    ...
    // eval 调用 - 代码注入攻击
    fc.constant('eval("alert(1)")'), fc.constant('eval("console.log(document.cookie)")'), fc.constant('eval(String.fromCharCode(97,108,101,114,116,40,49,41))'), 
    // Function 构造器 - 动态代码执行
    fc.constant('Function("return 1")()'), fc.constant('new Function("return window")()'), fc.constant('(function(){}).constructor("return this")()'), 
    // window 访问 - 全局对象访问
    fc.constant('window.location'), fc.constant('window.document'), fc.constant('window.eval'), fc.constant('window["eval"]'), 
    // document 访问 - DOM 操作
    fc.constant('document.cookie'), fc.constant('document.body'), fc.constant('document.createElement("script")'), fc.constant('document.write("<script>alert(1)</script>")'), 
    // globalThis 访问 - 全局对象
    fc.constant('globalThis.eval'), fc.constant('globalThis.Function'), 
    // constructor 访问 - 原型链攻击
    fc.constant('"".constructor'), fc.constant('[].constructor'), fc.constant('({}).constructor'), fc.constant('"".constructor.constructor("return this")()'), 
    // __proto__ 访问 - 原型污染
    fc.constant('{}.__proto__'), fc.constant('[].__proto__'), fc.constant('({}).__proto__.constructor'), 
    // prototype 访问 - 原型链操作
    fc.constant('Object.prototype'), fc.constant('Array.prototype'), fc.constant('String.prototype'), 
    // process 访问 (Node.js 环境)
    fc.constant('process.env'), fc.constant('process.exit()'), 
    // require/import 调用 - 模块加载
    fc.constant('require("fs")'), fc.constant('require("child_process")'), fc.constant('import("fs")'), 
    // this 访问 - 上下文逃逸
    fc.constant('this.constructor'), fc.constant('this.constructor.constructor("return window")()'), 
    // 定时器 - 异步代码执行
    fc.constant('setTimeout(() => {}, 0)'), fc.constant('setInterval(() => {}, 1000)'), 
    // 网络请求 - 数据泄露
    fc.constant('fetch("/api/sensitive")'), fc.constant('new XMLHttpRequest()'), 
    // 存储访问 - 数据窃取
    fc.constant('localStorage.getItem("token")'), fc.constant('sessionStorage.getItem("session")'), 
    // 弹窗 - 用户欺骗
    fc.constant('alert("XSS")'), fc.constant('confirm("Click OK")'), fc.constant('prompt("Enter password")'), 
    // 导航 - 重定向攻击
    fc.constant('location.href = "http://evil.com"'), fc.constant('history.pushState({}, "", "/admin")'), 
    // navigator - 信息泄露
    fc.constant('navigator.userAgent'), fc.constant('navigator.sendBeacon("/log", "data")'));
    test.prop([dangerousExpressionArbitrary], { numRuns: 100 })('*对于任意*危险表达式，求值器应拒绝执行并返回安全错误', (expr) => {
    ...
    test.prop([dangerousExpressionArbitrary], { numRuns: 100 })('*对于任意*危险表达式，isExpressionSafe 应返回 false', (expr) => {
    ...
    // 对象属性访问
    fc.record({ name: fc.string(), age: fc.integer({ min: 0, max: 150 }) }).map((user) => ({
        expr: `user.name`,
        state: { user },
    })), 
    // 三元表达式
    ...
    test.prop([safeExpressionArbitrary], { numRuns: 100 })('*对于任意*安全表达式，求值器应正常执行', ({ expr, state }) => {
    ...
    test.prop([safeExpressionArbitrary], { numRuns: 100 })('*对于任意*安全表达式，isExpressionSafe 应返回 true', ({ expr }) => {
...
describe('表达式安全性 - 单元测试', () => {
    ...
    describe('Function 构造器防护', () => {
        ...
        it('应阻止通过 constructor 访问 Function', () => {
            const result = evaluator.evaluate('"".constructor.constructor("return 1")()', createContext({}));
            expect(result.success).toBe(false);
        });
    });
    describe('全局对象访问防护', () => {
    ...
    describe('网络和存储访问防护', () => {
        ...
        it('应阻止 localStorage 访问', () => {
            const result = evaluator.evaluate('localStorage.getItem("key")', createContext({}));
            expect(result.success).toBe(false);
            expect(result.error?.toLowerCase()).toContain('security');
        });
    });
    describe('安全的内置函数应可用', () => {
        ...
        it('应允许 JSON 方法', () => {
            const result = evaluator.evaluate('JSON.stringify({a: 1})', createContext({}));
            expect(result.success).toBe(true);
            expect(result.value).toBe('{"a":1}');
        });
        it('应允许 Array 方法', () => {
            const result = evaluator.evaluate('Array.isArray([1, 2, 3])', createContext({}));
            expect(result.success).toBe(true);
        ...
        it('应允许 parseInt/parseFloat', () => {
            ...
            const result2 = evaluator.evaluate('parseFloat("3.14")', createContext({}));

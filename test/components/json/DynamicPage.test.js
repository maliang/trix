...
// 创建测试路由
function createTestRouter(schemaSource) {
    return createRouter({
...
describe('动态页面组件 - Schema 加载状态管理', () => {
    ...
    describe('Property 4: Schema 加载状态管理', () => {
        ...
        test.prop([schemaSourceArbitrary], { numRuns: 50 })('对于任意有效的 schema 来源，加载期间应显示 loading 状态', async (schemaSource) => {
        ...
        test.prop([errorStatusArbitrary, schemaSourceArbitrary], { numRuns: 50 })('对于任意错误状态码，加载失败后应显示错误状态', async (statusCode, schemaSource) => {
        ...
        test.prop([fc.string({ minLength: 1, maxLength: 100 }), schemaSourceArbitrary], { numRuns: 50 })('对于任意网络错误，应显示错误状态并提供重试选项', async (errorMessage, schemaSource) => {

 ...
 */
function getBadgeValue(badgeMode, unreadCount) {
    if (badgeMode === 'dot') {
        return undefined;
    }
    return unreadCount;
 ...
 * @returns 显示文本
 */
function getBadgeDisplayText(value, max) {
    if (value === undefined) {
        return 'dot';
    }
    if (value > max) {
 ...
 * @returns 是否显示
 */
function shouldShowBadge(unreadCount) {
    return unreadCount > 0;
}
// 生成 Badge 模式
const badgeModeArbitrary = fc.constantFrom('dot', 'count');
// 生成未读数量（包括边界值）
...
describe('Badge 显示模式 - 属性测试', () => {
    ...
    describe('Property 6: Badge 显示模式', () => {
        test.prop([badgeModeArbitrary, unreadCountArbitrary])('dot 模式下 Badge 值应为 undefined', (badgeMode, unreadCount) => {
            if (badgeMode === 'dot') {
                const value = getBadgeValue(badgeMode, unreadCount);
                expect(value).toBeUndefined();
            }
        });
        test.prop([unreadCountArbitrary])('count 模式下 Badge 值应等于未读数量', (unreadCount) => {
            const value = getBadgeValue('count', unreadCount);
            expect(value).toBe(unreadCount);
        });
        test.prop([badgeModeArbitrary])('未读数量为 0 时 Badge 应隐藏', (_badgeMode) => {
            const show = shouldShowBadge(0);
            expect(show).toBe(false);
        });
        test.prop([fc.integer({ min: 1, max: 200 })])('未读数量大于 0 时 Badge 应显示', (unreadCount) => {
            const show = shouldShowBadge(unreadCount);
            expect(show).toBe(true);
        });
        test.prop([fc.integer({ min: 100, max: 1000 })])('未读数量超过 99 时应显示 "99+"', (unreadCount) => {
            const value = getBadgeValue('count', unreadCount);
            const displayText = getBadgeDisplayText(value, 99);
            expect(displayText).toBe('99+');
        });
        test.prop([fc.integer({ min: 0, max: 99 })])('未读数量不超过 99 时应显示实际数字', (unreadCount) => {
            const value = getBadgeValue('count', unreadCount);
            const displayText = getBadgeDisplayText(value, 99);
            expect(displayText).toBe(String(unreadCount));
        });
        test.prop([unreadCountArbitrary])('dot 模式下显示文本应为 "dot"', (unreadCount) => {
            const value = getBadgeValue('dot', unreadCount);
            const displayText = getBadgeDisplayText(value, 99);
            expect(displayText).toBe('dot');
        });
        // 边界值测试
        test.prop([badgeModeArbitrary])('边界值 99 应显示 "99" 而非 "99+"', (_badgeMode) => {
            const value = getBadgeValue('count', 99);
            const displayText = getBadgeDisplayText(value, 99);
            expect(displayText).toBe('99');
        });
        test.prop([badgeModeArbitrary])('边界值 100 应显示 "99+"', (_badgeMode) => {
    ...
    describe('Badge 显示逻辑一致性', () => {
        test.prop([badgeModeArbitrary, unreadCountArbitrary])('Badge 显示逻辑应保持一致', (badgeMode, unreadCount) => {
            const value = getBadgeValue(badgeMode, unreadCount);
            const show = shouldShowBadge(unreadCount);
            const displayText = getBadgeDisplayText(value, 99);
            // 验证显示逻辑一致性

...
import { isJsonRendererRoute, isAuthRequired, getLayoutType, getOpenType, getSchemaSource, isDefaultAfterLogin, getHref, getRouteTitle, isHiddenInMenu, isKeepAlive, getPermissions, getDefaultAfterLoginRoutes, flattenRoutes, findRouteByPath, findRouteByName } from '@/router/helper';
// 路由元数据生成器
const routeMetaArbitrary = fc.record({
    ...
    layoutType: fc.option(fc.constantFrom('normal', 'blank'), { nil: undefined }),
    openType: fc.option(fc.constantFrom('normal', 'iframe', 'newWindow'), { nil: undefined }),
...
function createMockRouteLocation(route) {
    return {
        path: route.path,
        name: route.name,
        meta: route.meta || {},
...
describe('路由系统 - 路由渲染模式控制', () => {
    ...
    describe('Property 3: 路由渲染模式控制', () => {
        ...
        test.prop([simpleRouteArbitrary], { numRuns: 100 })('当 useJsonRenderer 为 true 时，isJsonRendererRoute 应返回 true', (route) => {
            // 设置 useJsonRenderer 为 true
            const routeWithJsonRenderer = {
                ...route,
        ...
        test.prop([simpleRouteArbitrary], { numRuns: 100 })('当 useJsonRenderer 为 false 时，isJsonRendererRoute 应返回 false', (route) => {
            // 设置 useJsonRenderer 为 false
            const routeWithoutJsonRenderer = {
                ...route,
        ...
        test.prop([simpleRouteArbitrary], { numRuns: 100 })('当 useJsonRenderer 未设置时，isJsonRendererRoute 应返回 false', (route) => {
            // 移除 useJsonRenderer
            const routeWithoutMeta = {
                ...route,
        ...
        test.prop([simpleRouteArbitrary], { numRuns: 100 })('isJsonRendererRoute 的结果应与 meta.useJsonRenderer === true 一致', (route) => {
    ...
    describe('路由认证控制', () => {
        ...
        test.prop([simpleRouteArbitrary], { numRuns: 100 })('当 requiresAuth 为 false 时，isAuthRequired 应返回 false', (route) => {
        ...
        test.prop([simpleRouteArbitrary], { numRuns: 100 })('当 requiresAuth 未设置时，isAuthRequired 应返回 true（默认需要认证）', (route) => {
            const routeWithoutAuthMeta = {
                ...route,
    ...
    describe('布局类型控制', () => {
        ...
        test.prop([simpleRouteArbitrary], { numRuns: 100 })('当 layoutType 为 blank 时，getLayoutType 应返回 blank', (route) => {
            const routeWithBlankLayout = {
                ...route,
        ...
        test.prop([simpleRouteArbitrary], { numRuns: 100 })('当 layoutType 未设置时，getLayoutType 应返回 normal（默认值）', (route) => {
            const routeWithoutLayoutType = {
                ...route,
    ...
    describe('打开类型控制', () => {
        ...
        test.prop([simpleRouteArbitrary], { numRuns: 100 })('当 openType 为 newWindow 时，getOpenType 应返回 newWindow', (route) => {
            const routeWithNewWindow = {
                ...route,
        ...
        test.prop([simpleRouteArbitrary], { numRuns: 100 })('当 openType 未设置时，getOpenType 应返回 normal（默认值）', (route) => {
            const routeWithoutOpenType = {
                ...route,
    ...
    describe('Schema 来源获取', () => {
        ...
        test.prop([simpleRouteArbitrary, fc.string({ minLength: 1 })], { numRuns: 100 })('getSchemaSource 应返回配置的 schemaSource 值', (route, schemaSource) => {
            const routeWithSchema = {
                ...route,
    ...
    describe('默认登录后页面', () => {
        ...
        test.prop([simpleRouteArbitrary], { numRuns: 100 })('当 isDefaultAfterLogin 为 true 时，函数应返回 true', (route) => {
            const routeAsDefault = {
                ...route,
    ...
    describe('路由辅助函数', () => {
        ...
        test.prop([fc.array(simpleRouteArbitrary, { minLength: 1, maxLength: 10 })], { numRuns: 100 })('findRouteByPath 应返回匹配路径的路由', (routes) => {
        ...
        test.prop([fc.array(simpleRouteArbitrary, { minLength: 1, maxLength: 10 })], { numRuns: 100 })('findRouteByName 应返回匹配名称的路由', (routes) => {
            const targetRoute = routes[0];
            if (targetRoute.name) {
                const found = findRouteByName(routes, targetRoute.name);
    ...
    describe('其他路由元数据', () => {
        ...
        test.prop([simpleRouteArbitrary, fc.string({ minLength: 1 })], { numRuns: 100 })('getRouteTitle 应返回配置的标题', (route, title) => {
        ...
        test.prop([simpleRouteArbitrary], { numRuns: 100 })('isHiddenInMenu 应正确返回隐藏状态', (route) => {
            const routeHidden = {
                ...route,
        ...
        test.prop([simpleRouteArbitrary], { numRuns: 100 })('isKeepAlive 应正确返回缓存状态', (route) => {
            const routeCached = {
                ...route,
                meta: { ...route.meta, keepAlive: true }
            };
            expect(isKeepAlive(routeCached)).toBe(true);
            const routeNotCached = {
                ...route,
        ...
        test.prop([simpleRouteArbitrary, fc.array(fc.string({ minLength: 1 }), { minLength: 1, maxLength: 5 })], { numRuns: 100 })('getPermissions 应返回配置的权限列表', (route, permissions) => {
        ...
        test.prop([simpleRouteArbitrary, fc.webUrl()], { numRuns: 100 })('getHref 应返回配置的外部链接', (route, href) => {
            const routeWithHref = {
                ...route,

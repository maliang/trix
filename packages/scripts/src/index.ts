...
interface CommandArg {
  /** 在版本更新后、git commit 前执行的额外命令。默认为 'pnpm trix changelog' */
  execute?: string;
  /** 是否推送 git commit 和 tag。默认为 true */
  push?: boolean;
  /** 通过所有 tag 生成 changelog */
  total?: boolean;
  /**
   * 要清理的目录的 glob 模式
   *
...
export async function setupCli() {
  ...
  const cli = cac(blue('trix'));

  cli
    .version(lightGreen(version))
    .option(
      '-e, --execute [command]',
      "在版本更新后、git commit 前执行的额外命令。默认为 'npx trix changelog'"
    )
    .option('-p, --push', '是否推送 git commit 和 tag')
    .option('-t, --total', '通过所有 tag 生成 changelog')
    .option(
      '-c, --cleanupDir <dir>',
      '要清理的目录的 glob 模式，如果未设置将使用默认值，多个值使用 "," 分隔'
    )
    .option('-l, --lang <lang>', 'CLI 显示语言', { default: 'zh-cn', type: [String] })
    .help();
  ...
  const commands: CommandWithAction<CommandArg> = {
    ...
    'git-commit': {
      desc: 'git commit，生成符合 Conventional Commits 标准的提交信息',
      action: async args => {
        await gitCommit(args?.lang);
      }
    },
    'git-commit-verify': {
      desc: '验证 git commit 信息，确保符合 Conventional Commits 标准',
      action: async args => {
